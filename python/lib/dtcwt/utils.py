...
def asfarray(X):
    ...
    """
    X = np.asanyarray(X)
    # NumPy 2.0 compatibility: asfarray was removed, use asarray instead
    try:
...
def appropriate_complex_type_for(X):
    ...
    # NumPy 2.0 compatibility: issubsctype was removed, use issubdtype instead
    try:
        is_complex64 = np.issubsctype(X.dtype, np.complex64)
        is_complex128 = np.issubsctype(X.dtype, np.complex128)
        is_float32 = np.issubsctype(X.dtype, np.float32)
    except AttributeError:
        is_complex64 = np.issubdtype(X.dtype, np.complex64)
        is_complex128 = np.issubdtype(X.dtype, np.complex128)
        is_float32 = np.issubdtype(X.dtype, np.float32)
    
    if is_complex64 or is_complex128:
        return X.dtype
    elif is_float32:
        return np.complex64
    else:
