...
class DtcwtKeyEncoder:
    ...
    def encode(self, img):
        ...
        for i in range(6):
            ...
            h, w = coeff.shape
            # NumPy 2.0 compatibility: use np.complex128 instead of 'complex_'
            coeffs = np.zeros(masks3[i].shape, dtype=np.complex128)
            coeffs[:h, :w] = coeff
            coeffs[-h:, :w] = coeff
            coeffs[:h, -w:] = coeff
            coeffs[-h:, -w:] = coeff
...
class DtcwtKeyDecoder:
    ...
    def decode(self, wmed_img):
        ...
        # NumPy 2.0 compatibility: use np.complex128 instead of 'complex_'
        coeffs = np.zeros((h, w, 6), dtype=np.complex128)
        for i in range(6):
